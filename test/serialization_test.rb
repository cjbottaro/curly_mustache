...
class SerializationTest < ActiveSupport::TestCase
  ...
  def test_proper_connection
    feed = Feed.create(:title => "blah", :url => "http://www.blah.com")
    assert_equal %w[BLOB], Feed.connection.adapter.get(Feed.send(:id_to_key, feed.id)).keys
    
    account = Account.create(:name => "blah", :created_at => Time.now, :updated_at => Time.now)
    assert_equal %w[id name created_at updated_at].sort, Account.connection.adapter.get(Account.send(:id_to_key, account.id)).keys.sort
    
    page = Page.create(:url => "http://www.blah.com", :html => "<blah></blah>")
    assert_equal %w[GLOB], Page.connection.adapter.get(Page.send(:id_to_key, page.id)).keys
  end
